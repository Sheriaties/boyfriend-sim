...
### 系统
- **顶部 HUD**：像素风分段进度条 + 4 个图标 + 时钟（`第N天 HH:MM`），右侧留 7vw 给回封面按钮
- **封面主界面**：`assets/cover.png`，"新游戏"/"载入游戏" 两个按钮（无存档时载入灰显）
- **回封面按钮**：右上角白底大 🏠 按钮，点击弹通用确认弹窗（是/否）
- **通用确认弹窗**：`showConfirm(text, onYes, onNo)` 复用
- **时间系统**：每次交互 +3 小时，>=22:00 男主自动说想睡觉 → 跳次日 07:00，压力-10 了解度+2
- **昼夜系统**：`gameTime` 6:00-18:00 白天 (`window_day.png`)，其他时间黑夜 (`window_night.png`)
- **20:00 自动关灯**：`state.autoLightsAdjusted` 每天只关一次，白天自动重置
- **卧室灯独立**：`state.lightsOn = {living, bedroom}`，客厅和卧室各有一个吊灯 + 开关热区
- **场景切换**：不消耗时间（`nav-arrow` 走 `data-nav`）
- **开场取名**：`heroName` 存 state，对话用 `showDialog('@bf', ...)` 自动替换
- **薯片 Bad End**：连喂 10 次，每次男主 scale ×1.05（≤ 1.65 封顶不出屏），第 10 次触发 fat 结局
- **点电脑弹三选项菜单**：
  - 🎮 玩游戏 → 打开 **2048 迷你游戏弹窗**（方向按钮/滑动，分数×0.1加心动）
    - 弹窗角落**任天堂彩蛋游戏机**：点 3 次触发对白 + 选择"一起玩/不玩"，一起玩显示 `cg_coop_game.png`
    - 只出现一次（`state.switchEggShown`）
  - 🎬 看剧 / 💼 工作：直接切坐姿 + 数值 + 推进时间
- **吊灯开关灯**：点吊灯弹菜单，暗色 overlay（客厅/卧室独立）
...
- **开发者面板**：`~`键 或 右上 ⚙ 按钮打开（跟 🏠 并排），可直接触发所有结局

### 结局系统（12 个）
- **判定入口**：`checkImmediateEnding()` 每次 `changeStat` 后调，`checkFinalEnding()` 每次 `advanceTime` 后调
- **播放屏**：图作背景（`pv_{id}.png`）+ 半透明黑色渐变叠加 + 文字面板；True End 挂 `.hopeful` class 用暖金希望配色
- **拦截交互**：`state.ended === true` 后所有 click 事件被拦截（除结局屏重来按钮和开发者面板）

**立即死亡（6 个）**：
| 条件 | id | 结局 |
|---|---|---|
| 压力=100 | `disappeared` | 👻 男主失踪 |
| 激情=100 压力<30 | `puppy` | 🐕 男朋友是我的小狗 |
| 激情=100 压力 30-59 | `yandere` | 🔗 病娇囚禁（被他锁） |
| 激情=100 压力≥60 | `exhausted` | 💀 虚脱 |
| 心动=0 激情<50 | `breakup_now` | 💔 分手（立即） |
| 心动=0 激情≥50 | `fwb` | 🔥 炮友 |

**4 周结算（5 个）**：
| 条件 | id | 结局 |
|---|---|---|
| 心动<50 | `breakup_final` | 💔 分手 |
| 心动≥50 激情<70 | `roommate` | 🛋 室友式同居 |
| 心动≥50 激情≥70 压力≥30 | `adversary` | ⚔️ 对抗路情侣 |
| 心动≥50 激情≥70 压力<30 了解<100 | `normal_end` | 💕 Normal 平淡长久 |
| 心动≥50 激情≥70 压力<30 了解=100 | `true_end` | ✨ True End 相互滋养 |
